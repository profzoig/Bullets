...
namespace Bullets
{
	public class AppMain
	{
		...
		private const int GAME_STATE_TITLE_INIT = 10;
		private const int  GAME_STATE_TITLE = 20;
		...
		public static void Main (string[] args)
		{
			...
			switchGameState (GAME_STATE_TITLE_INIT);
			gamePause = false;
			//Game Loop
			bool quitGame = false;
			while (!quitGame) {		
		...
		public static void gameStateTitleInit ()
		{
		...
		public static void spawnWave ()
		{
			...
			for (int i = 0; i < levelEnemyAmount; i++) {
				if (i == 0) {
					...
					pos = new Vector2 (800.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.5f);
				} else if (i == 3) {
					pos = new Vector2 (200.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.5f);
				} else if (i == 4) {
					pos = new Vector2 (600.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.4f);
				} else if (i == 5) {
					pos = new Vector2 (400.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.4f);
				} else if (i == 6) {
					pos = new Vector2 (600.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.6f);
				} else if (i == 7) {
					pos = new Vector2 (400.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.6f);
				} else if (i == 8) {
					pos = new Vector2 (500.0f, Director.Instance.GL.Context.GetViewport ().Height * 0.5f);
				}
		...
		public static void checkForEndOfLevel ()
		{
			...
			for (var i = 0; i < coinCol.Count; i++) {
				if (coinCol [i].isAlive == false) {
					allDead++;
				}
			}
			if (allDead == coinCol.Count) {
				switchGameState (GAME_STATE_NEW_LEVEL);
			}
		}
